...
function loadOverlay(marker) {
    ...
    img.onload = () => {
        ...
        // Store dimensions for replay coordinate transforms
        loc._overlayW = w;
        loc._overlayH = h;
...
const Replay = {
    ...
    STATE_COLORS: {
        ...
        correctingCourse: '#ffd60a',
        unsticking: '#e76f51',
        transitioning: '#3a86ff',
        evading: '#e63946',
        lost: '#9b2226',
        killed: '#6d6875',
        disconnected: '#555555'
    ...
    init() {
        ...
        document.getElementById('replayPlayPause').addEventListener('click', () => this.togglePlay());
        document.getElementById('replayStepBack').addEventListener('click', () => this.step(-1));
        document.getElementById('replayStepForward').addEventListener('click', () => this.step(1));
        document.getElementById('replayClose').addEventListener('click', () => this.close());
        document.getElementById('replaySpeed').addEventListener('change', e => {
            this.speed = parseFloat(e.target.value);
        });
        document.getElementById('replaySlider').addEventListener('input', e => {
            const pct = parseInt(e.target.value);
            const idx = Math.round(pct / 100 * (this.frames.length - 1));
            this.seekTo(idx);
        });
    },
    ...
    async loadFile(e) {
        ...
        this.recording = JSON.parse(text);
        ...
        const meta = this.recording.metadata || {};
        document.getElementById('replayTestName').textContent = meta.testName || file.name;
    ...
    getOverlayDims(data) {
        const loc = data.loc;
        if (loc._overlayW && loc._overlayH) return { w: loc._overlayW, h: loc._overlayH };
        // Fallback: load the image synchronously to get dimensions
        if (loc.imageFile && !loc._dimsPending) {
            loc._dimsPending = true;
            const img = new Image();
            const webpName = loc.imageFile.replace('.png', '.webp');
            img.onload = () => {
                loc._overlayW = img.naturalWidth / CONFIG.coordinates.scaleFactor;
                loc._overlayH = img.naturalHeight / CONFIG.coordinates.scaleFactor;
            };
            img.src = `data/tiles/images/${webpName}`;
        }
        // Default 512x512 minimap
        return { w: 2.56, h: 2.56 };
    },

    frameToMapCoords(frame) {
        const data = clusterDataMap[frame.clusterId];
        if (!data || !data.loc) return null;

        const dims = this.getOverlayDims(data);
        return CoordTransform.localToMapCoords(
            { x: frame.position.x, y: frame.position.y },
            data.loc, dims.w, dims.h
        );
    },
    ...
    renderFrame(frame) {
        ...
        if (coords) {
            this.botMarker.setLatLng(coords);
        }

        // Update target position
        if (frame.targetPosition) {
            const data = clusterDataMap[frame.clusterId];
            if (data && data.loc) {
                const dims = this.getOverlayDims(data);
                const targetCoords = CoordTransform.localToMapCoords(
                    { x: frame.targetPosition.x, y: frame.targetPosition.y },
                    data.loc, dims.w, dims.h
                );
                if (targetCoords) {
                    this.targetMarker.setLatLng(targetCoords);
                    this.targetMarker.setStyle({ opacity: 1, fillOpacity: 0.5 });
                }
            }
        } else {
        ...
        document.getElementById('replayTimeMs').textContent = (frame.timeMs / 1000).toFixed(1) + 's';
        document.getElementById('replayStateName').textContent = frame.state || '-';
        document.getElementById('replayStateName').style.color = stateColor;
        document.getElementById('replayCluster').textContent = frame.clusterId || '-';
    ...
    updateSlider() {
        ...
        const pct = this.frames.length > 1
            ? (this.currentFrameIndex / (this.frames.length - 1)) * 100
            : 0;
        slider.value = Math.round(pct);
    },

    updateLogs() {
        const panel = document.getElementById('replayLogPanel');
        const currentFrame = this.frames[this.currentFrameIndex];
        if (!currentFrame) return;

        const currentTimeMs = currentFrame.timeMs;

        // Show logs up to current time
        const visibleLogs = this.logs.filter(l => l.timeMs <= currentTimeMs);
        const recentLogs = visibleLogs.slice(-50); // Keep last 50

        panel.innerHTML = recentLogs.map(l => {
            const levelClass = l.level === 'Warning' ? 'warn'
                : l.level === 'Error' ? 'error'
                : l.level === 'Information' ? 'info' : 'debug';
            const time = (l.timeMs / 1000).toFixed(1);
            return `<div class="replay-log-entry ${levelClass}">` +
                `<span class="replay-log-time">${time}s</span>` +
                `<span class="replay-log-msg">${this.escapeHtml(l.message)}</span></div>`;
        }).join('');
    ...
    play() {
        ...
        this.playing = true;
        document.getElementById('replayPlayPause').textContent = 'Pause';
    ...
    pause() {
        this.playing = false;
        document.getElementById('replayPlayPause').textContent = 'Play';
        if (this.animFrameId) {
    ...
    step(delta) {
        this.pause();
        this.seekTo(this.currentFrameIndex + delta);
    },

    playbackLoop(now) {
        if (!this.playing) return;

        const dt = (now - this.lastTickTime) * this.speed;
        ...
        // Time between frames in recording ms
        const frameDeltaMs = nextFrame.timeMs - currentFrame.timeMs;

        // Accumulate real elapsed time and advance frames accordingly
        this._accumMs = (this._accumMs || 0) + dt;

        while (this._accumMs >= frameDeltaMs && this.currentFrameIndex < this.frames.length - 1) {
            this._accumMs -= Math.max(frameDeltaMs, 1);
            this.currentFrameIndex++;

            // Check for cluster change — pan if needed
            if (this.currentFrameIndex > 0) {
                ...
                if (prev.clusterId !== curr.clusterId) {
                    this.panToFrame(curr);
                    this.ensureOverlays([curr.clusterId]);
                }
            }
            ...
            // Recalculate frameDeltaMs for next iteration
            const cF = this.frames[this.currentFrameIndex];
            const nF = this.frames[this.currentFrameIndex + 1];
            if (nF) {
                const newDelta = nF.timeMs - cF.timeMs;
                if (this._accumMs < newDelta) break;
            }
        }
    ...
    close() {
        ...
        document.getElementById('replayFileInput').value = '';
    }
};

// Initialize replay on DOMContentLoaded
